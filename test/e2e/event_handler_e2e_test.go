...
func TestE2e_SuccessMockEventHandler(t *testing.T) {
...
func TestE2e_SuccessEventHandler(t *testing.T) {
	testID := ksuid.New().String()
	// Setup
	config := es.NewConfig()
	db := database.Connect()
	...
	repository := es.NewAggregateRepository(config, db, eventRegistry)
	eventLogger := NewEventLogger(config, db)
	repository.Subscribe("completed_event", eventLogger)
	...
	fmt.Println("TestE2e_SuccessEventHandler: done")
}
